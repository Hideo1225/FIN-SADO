$(function(){
    ...
    /*
   クラス名sliderにスライドショーを適用（index02の14行目~18行目）
   slick.jsというプラグインを使用（index02の27行目でslick.jsを読み込み）
    ...
    */
    ...
    if(document.URL.match(/index03/)){                      //index03を表示した際に起動
        const img = document.getElementById('flour-photo'); //IDがflour-photoの要素を取得（index03の14行目）
        ...
        const title = document.getElementById('title');     //IDがtitleの要素を取得（index03の18行目）
    ...
    if(document.URL.match(/index04/)){                          //index04を表示した際に起動
        const title = document.getElementById('title');         //IDがtitleの要素を取得（index04の15行目）
        const sub_title = document.getElementById('sub-title'); //IDがsub-titleの要素を取得（index04の14行目）
